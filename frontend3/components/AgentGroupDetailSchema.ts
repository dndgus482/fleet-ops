...
export function useAgentGroupForm() {
  const { validateField, meta, values } = useForm({
  ...
  async function validateForm(): Promise<boolean> {
    ...
    return results.every(v => v.valid)
  }

  return { validateForm, meta, values }
}
...
export function useAgentField() {
  ...
  const errorMessage = computed(() => {
    return agentError.value ?? addAgentError.value
  })

  const isAddButtonEnabled = computed(() => {
    return ipInput.value && userNameInput.value && !addAgentError.value
  })
